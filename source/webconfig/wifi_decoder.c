    ...
    em_per_radio_report_t *radio_report)
{
    ...
    for (j = 0; j < cJSON_GetArraySize(param_arr); j++) {
        ...
        if (assoc_sta_arr != NULL && cJSON_IsArray(assoc_sta_arr)) {
            ...
            if (radio_report->vap_reports[j].sta_traffic_stats == NULL) {
                wifi_util_error_print(WIFI_WEBCONFIG, "%s:%d: failed to allocate memory\n", __func__, __LINE__);
                return webconfig_error_decode;
            }
        ...
        if (assoc_sta_arr != NULL && cJSON_IsArray(assoc_sta_arr)) {
            ...
            if (radio_report->vap_reports[j].sta_link_metrics == NULL) {
                wifi_util_error_print(WIFI_WEBCONFIG, "%s:%d: failed to allocate memory\n", __func__, __LINE__);
                return webconfig_error_decode;
            }
            for (sta_cnt = 0; sta_cnt < cJSON_GetArraySize(assoc_sta_arr); sta_cnt++) {
                ...
                if (value_object->valuestring != NULL && value_object->valuestring[0] != '\0') {
                    strncpy(psm->client_type, value_object->valuestring, sizeof(psm->client_type) - 1);
                    psm->client_type[sizeof(psm->client_type) - 1] = '\0';
                } else {
                    psm->client_type[0] = '\0'; // Set to empty string if value is NULL
                }
