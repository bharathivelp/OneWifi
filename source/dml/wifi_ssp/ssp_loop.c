...
void Psm_Db_Write_MacFilter(wifi_mac_entry_param_t *mcfg)
{
    ...
    char *mcfg_mac;
    wifi_util_dbg_print(WIFI_PSM, "%s:%d bharathi enter\n", __func__, __LINE__);
    ...
    if (ret == RETURN_OK) {
        ...
        if (strlen(mcfg->mac) != 0) {
            ...
            if (temp_mac_entry != NULL) {
                ...
                wifi_util_dbg_print(WIFI_PSM, "%s:%d bharathi mac entry already present\r\n",__func__, __LINE__);
                free(mcfg_mac);
                return;
            }
            ...
            if (ret == RETURN_OK) {
                ...
                if (temp_mac_entry == NULL) {
                    wifi_util_dbg_print(WIFI_PSM, "%s:%d bharathi malloc failure\r\n",__func__, __LINE__);
                    free(mcfg_mac);
                    return;
                }
                ...
                {
                    wifi_util_error_print(WIFI_PSM, "%s:%d bharathi hash_map_put failed for mac filter entry\r\n", __func__, __LINE__);
                    return;
                }
                mcfg_mac = NULL;
    ...
    wifi_util_info_print(WIFI_PSM, "%s:%d bharathi exit update mac filter done\n", __func__, __LINE__);
}
